...
def process_and_add_to_config(file_path, channel_ids):
    ...
    config['exclude_from'] = list(exclude_from | not_found)
    config['to'] = list(to | not_found)
...
channel_ids_to_add = input_channel_ids()
...
if os.path.exists(file_path):
    config, not_found = process_and_add_to_config(file_path, channel_ids_to_add)
    results.append((file_path, config, not_found))
else:
    print(f"File {file_path} does not exist.")

for file_path, config, not_found in results:
